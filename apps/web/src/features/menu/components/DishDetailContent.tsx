...
interface Props {
  dishId: string;
  /** Llamado tras añadir al carrito; típico para cerrar el bottom sheet. */
  onAdded?: () => void;
}

/**
 * Contenido reutilizable del detalle de plato. Igual lógica y UI que la
 * página `/dish/[dishId]`, pero extraído para poder montarlo dentro de un
 * BottomSheet sin navegar de ruta.
 */
export function DishDetailContent({ dishId, onAdded }: Props) {
  ...
  return (
    <div className="relative">
      ...
      <div
        className="sticky bottom-0 left-0 right-0 border-t border-border bg-background/95 px-4 pt-3 backdrop-blur supports-[backdrop-filter]:bg-background/80"
      ...
      >
        <div className="mx-auto flex max-w-2xl items-center gap-3">
          <div className="flex items-center gap-1.5" role="group" aria-label="Cantidad">
            ...
            <span className="w-7 text-center text-body font-semibold tabular-nums">
              {quantity}
            </span>
            <button
              type="button"
